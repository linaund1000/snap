...
namespace GPOyun.Tests
{
    public class GlobalInputAndUISystemTests
    {
        ...
        public IEnumerator NewspaperBoardUI_Toggle_PausesAndResumesGame()
        {
            ...
            Assert.AreEqual(GameManager.GameState.Paused, GPOyun.Core.ServiceLocator.Get<GPOyun.Core.GameManager>().CurrentState, "Game should be paused when board is active");
            if (!isBatchMode)
            {
            ...
            Assert.AreEqual(GameManager.GameState.Playing, GPOyun.Core.ServiceLocator.Get<GPOyun.Core.GameManager>().CurrentState, "Game should resume playing when board is closed");
            if (!isBatchMode)
            {
