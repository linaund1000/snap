...
namespace GPOyun.NPC.UtilityAI
{
    public class GossipAction : MoveAction
    {
        ...
        {
            if (GPOyun.Core.ServiceLocator.Get<GPOyun.Core.RelationshipMatrix>() == null) return null;
            ...
            foreach (var hit in hits)
            {
                ...
                {
                    if (GPOyun.Core.ServiceLocator.Get<GPOyun.Core.RelationshipMatrix>().GetRelationship(Controller.NpcId, otherNpc.NpcId) > 20)
                    {
                        return otherNpc;
                    }
                }
