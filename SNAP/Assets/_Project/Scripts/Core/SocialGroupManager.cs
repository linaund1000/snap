...
namespace GPOyun.Core
{
    public class SocialGroup
    {
        ...
        public SocialGroup(string id, Vector3 position, int maxMembers = 4)
        {
            GroupId = id;
        ...
        public SocialGroup TryJoinNearbyGroup(NPCController joiner, float maxDistance = 15f)
        {
            ...
            foreach (var group in _activeGroups)
            {
                ...
                foreach (var member in group.Members)
                {
                    ...
                    {
                        affinity += GPOyun.Core.ServiceLocator.Get<GPOyun.Core.RelationshipMatrix>().GetRelationship(joiner.NpcId, member.NpcId);
                    }
                }
