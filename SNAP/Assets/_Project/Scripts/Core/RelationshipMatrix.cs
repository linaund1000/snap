...
namespace GPOyun.Core
{
    ...
    public class RelationshipMatrix : MonoBehaviour
    {
        ...
        private void Awake()
        {
            ServiceLocator.Register<RelationshipMatrix>(this);
        }

        private void OnDestroy()
        {
            ServiceLocator.Unregister<RelationshipMatrix>();
        }
        ...
        public int GetFavorite(int npcId)
        {
            ...
            foreach (var kvp in _relationsTable)
            {
                if (kvp.Key.A == npcId && kvp.Value > highestScore)
                {
                    highestScore = kvp.Value;
                    favoriteId = kvp.Key.B;
                }
                else if (kvp.Key.B == npcId && kvp.Value > highestScore)
                {
                    highestScore = kvp.Value;
                    favoriteId = kvp.Key.A;
                }
            }
            return favoriteId;
        }
