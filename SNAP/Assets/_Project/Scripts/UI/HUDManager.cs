...
namespace GPOyun.UI
{
    ...
    public class HUDManager : MonoBehaviour
    {
        ...
        private void DrawCozyScoreboard()
        {
            ...
            foreach (var npc in npcs)
            {
                ...
                foreach (var other in npcs)
                {
                    ...
                    int score = GPOyun.Core.ServiceLocator.Get<GPOyun.Core.RelationshipMatrix>() != null ? GPOyun.Core.ServiceLocator.Get<GPOyun.Core.RelationshipMatrix>().GetRelationship(npc.NpcId, other.NpcId) : 0;
        ...
        {
            if (clockText != null && GPOyun.Core.ServiceLocator.Get<GPOyun.Managers.TimeManager>() != null)
            {
                clockText.text = GPOyun.Core.ServiceLocator.Get<GPOyun.Managers.TimeManager>().GetFormattedTime();
            }
        }
